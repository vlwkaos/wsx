...
for target in "${TARGETS[@]}"; do
  ...
  MACOSX_DEPLOYMENT_TARGET=$deployment_target \
    cargo +"$TOOLCHAIN" build --release --locked --target "$target" -p wsx \
    --manifest-path "$ROOT/Cargo.toml"
  MACOSX_DEPLOYMENT_TARGET=$deployment_target \
    CARGO_TARGET_DIR="$ROOT/target/herdr-$target" \
    ZIG="$ZIG_BIN" \
    cargo +"$TOOLCHAIN" build --release --locked --target "$target" \
    --manifest-path "$ROOT/vendor/herdr/Cargo.toml"
done

lipo -create -output "$STAGE/wsx" \
  "$ROOT/target/aarch64-apple-darwin/release/wsx" \
  "$ROOT/target/x86_64-apple-darwin/release/wsx"
lipo -create -output "$STAGE/herdr" \
  "$ROOT/target/herdr-aarch64-apple-darwin/aarch64-apple-darwin/release/herdr" \
  "$ROOT/target/herdr-x86_64-apple-darwin/x86_64-apple-darwin/release/herdr"

cp "$ROOT/LICENSE" "$STAGE/LICENSE-wsx"
cp "$ROOT/vendor/herdr/LICENSE" "$STAGE/LICENSE-herdr"
cp "$ROOT/vendor/herdr/vendor/libghostty-vt/LICENSE" "$STAGE/LICENSE-libghostty-vt"
cp "$ROOT/vendor/herdr/vendor/portable-pty/LICENSE.md" "$STAGE/LICENSE-portable-pty.md"
