...
TUI에서는 프로젝트를 펼치면 비어 있지 않은 `Routines` 섹션이 나타납니다. 프로젝트나 루틴에서 `u`로 생성하고, `F1`/`F2`로 편집 가능한 Codex/Claude 초기값을 적용하며, `e`로 수정하고 확인된 `d`로 삭제합니다. 실행 중 삭제는 먼저 취소합니다. command argv는 shell 문자열이 아닌 JSON 배열로 편집합니다. 미리보기에는 설정, 다음/최근 실행, 로그 경로, capabilities, 최종 agent output이 표시되며 모바일에서는 Enter로 전체 화면 상세를 엽니다.
