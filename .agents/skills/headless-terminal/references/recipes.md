...
Examples assume `.work/ht/<run>/` exists and every short session name is replaced with a unique current-run-owned name. Use another path only when the user explicitly requests a durable artifact.

## Edit a file with vim

```
ht run --name v vim .work/ht/<run>/notes.md
ht send --wait-idle 100ms v "ihello world<Esc>:wq<CR>"
ht wait --exit v
ht remove v
cat .work/ht/<run>/notes.md
```
...
When scripted automation hits an unexpected dialog, inspect it first and send only a dismissal key whose effect is understood. Never send `<CR>` speculatively because some TUIs treat it as confirmation of a destructive action.
...
```
ht run --cwd .work/ht/<run> --env FOO=bar --env DEBUG=1 --name s my-tui
```
...
```
ht view --format png --output .work/ht/<run>/session.png S
```
