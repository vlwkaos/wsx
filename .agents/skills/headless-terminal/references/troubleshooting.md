...
Start with `ht list` and `ht view <sid>`. Act only on a session owned by the current run. Try `ht stop <sid>` followed by `ht remove <sid>` first. Use `ht kill` or `ht remove --force` only with explicit approval unless graceful cleanup failed for a current-run-owned session.

Do not stop the shared `ht` daemon while unrelated sessions exist. If it appears wedged, report the affected sessions and ask before `ht daemon stop`.
...
2. If a stale socket may remain after a crash, inspect its owner and active sessions. Do not remove it manually without explicit approval.
3. Permissions: the socket inherits your UID's umask. If another user owns a leftover socket, stop and report it.
