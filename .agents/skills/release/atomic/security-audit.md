...
For a real secret or sensitive file: stop, revoke/rotate as needed, remove it from tracking (for example `git rm --cached <file>` when the local file should remain), and consider history cleanup. Re-run the audit after remediation. If a finding is demonstrably non-secret, record the evidence and continue without user confirmation. Ask only when classification remains ambiguous or the response would change security posture. Never print a secret beyond the minimum already exposed by a command.
