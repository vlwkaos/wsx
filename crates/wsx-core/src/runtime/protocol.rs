...
mod tests {
    ...
    fn lifecycle_control_is_additive_and_tagged() {
        ...
        let handoff = Request::PrepareHandoff {
            target_binary_id: "0.25.0:1:2:3:4".into(),
            target_version: "0.25.0".into(),
            target_protocol: PROTOCOL_VERSION,
            target_daemon_revision: DAEMON_REVISION,
            executable: "/usr/local/bin/wsxd".into(),
        };
        let encoded = serde_json::to_string(&handoff).unwrap();
        assert_eq!(serde_json::from_str::<Request>(&encoded).unwrap(), handoff);
    ...
    fn legacy_snapshot_defaults_missing_foreground_job_metadata() {
        ...
        assert!(!snapshot.capabilities.live_handoff);
    }
