...
# WSX_INTEGRATION_VERSION=6
import os, subprocess

def _report(**kw):
    pane=os.environ.get("WSX_PANE_ID"); sid=kw.get("session_id")
    if not pane or not isinstance(sid,str) or not sid: return
    try: subprocess.run([os.environ.get("WSX_AGENT_REPORT_BIN") or "wsx","agent","report",pane,"--provider","hermes","--state","unknown","--session-id",sid],timeout=1,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL)
