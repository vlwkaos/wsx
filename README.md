...
Use `wsx group ls|create|rename|add|remove` to manage groups. Status, worktree-list, and session-list accept one `--group <name>`. The TUI starts with no group filter; group changes remain available for the running session. Legacy tab configuration and temporary multi-selection cache data migrate once and are rewritten; tab commands and flags are removed. Staleness uses trusted `wsx agent report` and terminal-entry activity, but wsx never infers vendors or semantic activity from processes or terminal output.
...
Native agent conversation restoration is enabled by default. Set `resume_agents_on_restore = false` to preserve generic saved launch recipes after wsxd restarts. `auto_collapse_after_hours` defaults to `24`; set it to `0` to disable automatic project collapse. A malformed or unreadable global config disables restoration for that startup. Changes apply after wsxd restarts.
