...
- **wsxd is runtime authority**: wsxd owns sessions, panes, PTYs, Ghostty state, frames, leases, revisions, and mutations. Clients own projection, selection, rendering, and local mute. Never add process-tree, pane-motion, bell, or terminal-text agent-state inference.
...
- **Protocol evolution**: Additive wire fields must deserialize with defaults so a new client can read an older daemon's Hello before deciding to upgrade it. Compatibility tests must feed raw legacy JSON that omits the new field; serializing fixtures with current structs cannot prove backward decoding.
