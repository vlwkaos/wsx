...
fn load_conpty() -> ConPtyFuncs {
    ...
    if std::env::var("WSX_WINDOWS_CONPTY").as_deref() == Ok("system") {
        log::warn!("WSX_WINDOWS_CONPTY=system disables wsx's bundled ConPTY");
        return system;
    }
    ...
    match app_local_conpty_path() {
        Ok(Some(path)) => load_app_local_conpty(&path).unwrap_or_else(|error| {
            panic!(
                "failed to load verified app-local ConPTY from {}: {error}; set WSX_WINDOWS_CONPTY=system to use the Windows system ConPTY",
                path.display()
            )
        }),
        Ok(None) => system,
        Err(error) => panic!(
            "wsx's app-local ConPTY bundle is invalid: {}; reinstall wsx or set WSX_WINDOWS_CONPTY=system to use the Windows system ConPTY",
            error
        ),
    }
}
...
fn app_local_conpty_path() -> Result<Option<PathBuf>, String> {
    ...
    let executable = std::env::current_exe()
        .map_err(|error| format!("cannot locate the wsx executable: {error}"))?;
    let executable_dir = executable
        .parent()
        .ok_or_else(|| "wsx executable has no parent directory".to_string())?;
    ...
    let mut expected = BTreeSet::from(["wsx-conpty.json".to_string()]);
    ...
    let marker = bundle.join("wsx-conpty.json");
